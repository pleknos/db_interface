  ...
  async componentWillMount () {
    ...
    for (let column of columns) {
      if (column.content_table) {
        column.contents =
          await Postgres.select({place: 'public.' + [column.content_table], director: 'id', direction: 'ASC'})
      }
    }
  ...
  render () {
    ...
    return (
      ...
      <div>
        ...
        <OutputTable shouldReloadRows={this.state.shouldReloadRows}
        ...
        <FilterModal onClick={this.handleModalToggle} columns={this.state.columns} filter={this.filter}
                     reloadRows={this.reloadRows} />}
      </div>
    )
  }
