...
export default class OutputTable extends React.Component {
  ...
  handleTableScroll(event) {
    ...
    if (target.scrollHeight - target.clientHeight - target.scrollTop === 0 && target.scrollTop !== 0) this.loadRows(100)
  }
  ...
  render() {
    ...
    const tableHeaderCols = columns.map(column => <th key={column.name}>{column.literal}</th>)
    ...
    return (
      <div>
        ...
        {this.state.changeRowModal && (
          <ChangeRowModal
            onClick={this.initiateChangeRow}
            contents={this.state.changeRowModalContents}
