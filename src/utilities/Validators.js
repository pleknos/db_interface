...
export const validateSingle = (value, column) => {
  ...
  if (column.type === 'table' && (tc === '' || tc === null))
    throw new ValidationError(`Отсутствует значение в поле '${column.literal}'`)

  if (column.type === 'date') {
    if (!/^[0-9]{1,2}[/][0-9]{1,2}[/][0-9]{2,4}$/g.test(tc)) throw new ValidationError(`Дата введена неправильно`)
  }
