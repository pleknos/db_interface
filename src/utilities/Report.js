...
export default class Report {
  ...
  async createReport() {
    ...
    for (let activity of this.activities) {
      let { amounts, data } = this.filterAndFill(
        [wholeFact, wholePlan, beforeFact, beforePlan],
        'activity_type',
        activity
      )
  ...
  async fillContents(contents, counter, reportObject, prevData) {
    ...
    for (let col of tableCols) {
      let { amounts, data } = this.filterAndFill(prevData, tableCol, col.id)
